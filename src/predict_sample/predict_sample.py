...
async def load_data_and_get_sample(request: DataModelRequest):
    ...
    # Should later add to the Pydantic Model the Predictor model, default Random Forest Classification
    ...
    # Send request to predict sample endpoint. 
    # (Working Python requests implementation. Httpx client can be tried)

    try: 
        r = requests.post(
                "http://data-simulation-api:8004/predict_sample_v2",  # Using service name
                 params={"input_json": json.dumps(data_dict)}, 
                 timeout = 5
            )
        logger.info("Request line completed") 
        #r.raise_for_status() 
    ...
    # Impement the return dict based on the response from the prediction_endpoint
    return_dict = r.json()
    return return_dict
...
async def make_prediction_v2(input_json: str):
    # Modified the Pydantic model to add one sample of feature data.
    ...
    rand_target = request["Label"] #This is the True Label

    logger.info(f"Values of feature vector :{x_sample}")
