...
logger = logging.getLogger("update")  # ATTENTION: This is necessary to evaluate the logging-messages in the test_update.py script?! Then everything must be rewritten to logger.info
app = FastAPI()
...
async def update_deployment_model(request: UpdateModelRequest):  # no instantiation here
    ...
    try:
        ...
        if versions:
            ...
            logger.error("Error during updating: model / dataset combination could not be found in the MLFlow model registry")
            return {"Error during updating": "model / dataset combination could not be found in the MLFlow model registry"}

        best_version = None
        best_metric = float('-inf')
        logger.info(f"Versions found in set_deployment_alias: {versions}")

        for version in versions:
            run_id = version.run_id
            logger.info(f"run_id from function set_deployment_alias: {run_id}")
            run = client.get_run(run_id)
            logger.info(f"run from client.get_run() inside set_deployment_alias(): {run}")
            metrics = run.data.metrics
            logger.info(f"metrics from run.data.metrics inside set_deployment_alias: {metrics}")
            ...
            if metric_name in metrics:
                ...
                logger.error("Error during updating: metric_name is not defined / available")
                return {"Error during updating": "metric_name is not defined / available"}
                
        if best_version:
            logger.info(f"Best Version found from function set_deployment alias: {best_version}")
            ...
            for version in versions:
                if version.version != best_version:
                    ...
                    logger.info(f"Set 'not_deployment' alias for version {version.version}")
            
            # Set "deployment" alias for the best version
            client.set_registered_model_alias(model_name, "deployment", best_version)
            logger.info(f"Set version {best_version} as deployment for model {model_name}")
        else:
            logger.info("No suitable model found to set as deployment")
